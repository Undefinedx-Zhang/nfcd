...
def main(config, resume, gpu, aug_type='all'):
    ...
    dataset = config['train_supervised']['data_dir'].split("/")[-1]
    ...
    # config['third_stage_loader']['aug_type'] = aug_type
    ...
    # config['third_stage_loader']['percnt_lbl'] = config["percent"]
    ...
    # third_stage_loader = dataloaders.CDDataset(config['third_stage_loader'])

    config['nf_train_supervised']['percnt_lbl'] = config["percent"]
