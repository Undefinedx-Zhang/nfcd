...
class BaseDataSet(Dataset):
    ...
    def _nf_augmentation(self, image_A, image_B, label):
        ...
        self.transform_x = T.Compose([
            T.Resize(self.base_size, Image.Resampling.LANCZOS),
            T.RandomRotation(5),
        ...
        image_A = self.transform_x(image_A)
        image_B = self.transform_x(image_B)

        # Normalize the images
        return self.normalize(image_A), self.normalize(image_B), label
    ...
    def __getitem__(self, index):
        ...
        elif self.augment == 'nf':
            image_A, image_B, label = self._nf_augmentation(image_A, image_B, label)
            label[label>=1] = 1
            label = torch.from_numpy(np.array(label, dtype=np.int32)).long()
            return image_A, image_B, label
