...
def batch_binary_classes(output, target):
    _, predict = torch.max(output, 1)

    predict = predict.int()
    target = target.int()

    TP = ((predict==1).float() * (target==1).float()).sum()
    FP = ((predict==1).float() * (target==0).float()).sum()
    TN = ((predict==0).float() * (target==0).float()).sum()
    FN = ((predict==0).float() * (target==1).float()).sum()

    return TP.cpu().numpy(), FP.cpu().numpy(), TN.cpu().numpy(), FN.cpu().numpy()
...
def eval_metrics(output, target, num_classes):
    ...
    tp, fp, tn, fn = batch_binary_classes(output, target) 
    
    return [np.round(correct, 5), np.round(labeled, 5), np.round(inter, 5), np.round(union, 5), tp, fp, tn, fn]
...
def compute_intersection_union(pred, target):
    # Convert to boolean tensors if they aren't already
    pred, target = pred.bool(), target.bool()
    
    # Calculate intersection and union
    intersection = torch.logical_and(pred, target).sum(dim=[1, 2])  # Sum over height and width dimensions
    union = torch.logical_or(pred, target).sum(dim=[1, 2])  # Sum over height and width dimensions
